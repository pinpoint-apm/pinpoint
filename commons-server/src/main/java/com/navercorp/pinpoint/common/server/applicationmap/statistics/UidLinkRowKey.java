...
public class UidLinkRowKey implements RowKey {
    ...
    public static UidLinkRowKey read(int saltKey, byte[] bytes) {
        ...
        buffer.setOffset(saltKey);
        String applicationName = buffer.readPadStringAndRightTrim(PinpointConstants.UID_SERVICE_NAME_LEN);
        int applicationServiceType = buffer.readInt();
        int serviceUid = buffer.readInt();
        long timestamp = TimeUtils.recoveryTimeMillis(buffer.readLong());
