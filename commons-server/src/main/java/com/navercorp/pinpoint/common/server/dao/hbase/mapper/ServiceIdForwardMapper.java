 ...
 */
@Component
public class ServiceIdForwardMapper implements RowMapper<ServiceInfo> {
    ...
    public ServiceInfo mapRow(Result result, int rowNum) throws Exception {
        ...
        UUID serviceIdValue = BytesUtils.bytesToUUID(cell.getRowArray(), cell.getRowOffset());
        ServiceId serviceId = ServiceId.of(serviceIdValue);
        String serviceName = BytesUtils.toString(cell.getValueArray(), cell.getValueOffset(), cell.getValueLength());

        return new ServiceInfo(serviceId, serviceName);
    }
