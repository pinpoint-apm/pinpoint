...
public class AgentStatModelConverter<T extends AgentStatDataPoint> {
    ...
    public static List<AgentStat> convertProfilerMetricToAgentStat(ProfilerMetricBo profilerMetricBo) {
        List<AgentStat> agentStats = new ArrayList<>();
        List<Tag> tags = new ArrayList<>();
        profilerMetricBo.getTags().forEach((key, value) -> {tags.add(new Tag(key, value));});
        String agentId = profilerMetricBo.getAgentId();
        long timestamp = profilerMetricBo.getTimestamp() * 1000;
        String metricName = profilerMetricBo.getMetricName();
        profilerMetricBo.getValues().forEach((key, value) -> {
            AgentStat stat = new AgentStat("defaultTenantId", "", "applicationName", agentId, metricName, key, value, timestamp, tags);
            agentStats.add(stat);
        });
        return agentStats;
    }
