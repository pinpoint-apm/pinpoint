...
export class ServerAndAgentListContainerComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.selectedSortOptionKey =
      (this.webAppSettingDataService.getAgentListSortOption() as SortOption) ||
      SortOption.ID;
    ...
    merge(
      this.newUrlStateNotificationService.onUrlStateChange$.pipe(
        ...
        tap((urlService: NewUrlStateNotificationService) => {
          ...
          const isPeriodChanged =
            urlService.isValueChanged(UrlPathId.PERIOD) ||
            urlService.isValueChanged(UrlPathId.END_TIME);
        ...
        map((urlService: NewUrlStateNotificationService) => {
          if (urlService.isRealTimeMode()) {
            ...
            return [
              urlService.getStartTimeToNumber(),
              urlService.getEndTimeToNumber(),
            ];
          }
        })
      ),
      this.messageQueueService
        .receiveMessage(this.unsubscribe, MESSAGE_TO.FETCH_AGENT_LIST)
        .pipe(
      ...
      .pipe(
        filter(() =>
          this.newUrlStateNotificationService.hasValue(UrlPathId.APPLICATION)
        ), // prevent from getting event after the component has been destroyed
        concatMap((range: number[]) => {
          ...
          const app = (
            urlService.getPathValue(UrlPathId.APPLICATION) as IApplication
          ).getApplicationName();
          ...
          return iif(
            () =>
              isEmpty(this.filteredServerList) ||
              isAppChanged ||
              isPeriodChanged ||
              isRealTimeMode,
            this.serverAndAgentListDataService
              ...
              .pipe(
                ...
                filter(
                  (res: IServerAndAgentDataV2[] | IServerErrorShortFormat) => {
                    if (
                      isThatType<IServerErrorShortFormat>(
                        res,
                        "errorCode",
                        "errorMessage"
                      )
                      ...
                      this.messageQueueService.sendMessage({
                        to: MESSAGE_TO.IS_ACCESS_DENYED,
                        param: true,
                      });
                      return false;
                    } else {
      ...
      .subscribe((data: IServerAndAgentDataV2[]) => {
        ...
        this.filteredServerList = this.filterServerList(
          this.serverList,
          this.query
        );
        this.filteredServerKeyList = this.filteredServerList
          .map(({ groupName }: IServerAndAgentDataV2) => groupName)
          .sort();
  ...
  private initI18nText(): void {
    ...
    this.inputPlaceholder$ = this.translateService
      .get("COMMON.MIN_LENGTH")
      .pipe(
        map((text: string) =>
          this.translateReplaceService.replace(text, this.SEARCH_MIN_LENGTH)
        )
      );
  }
  ...
  private filterServerList(
    serverList: IServerAndAgentDataV2[],
    query: string,
    predi?: (data: IAgentDataV2) => boolean
  ): IServerAndAgentDataV2[] {
    const filterCallback = predi
      ? predi
      : ({ agentId, agentName }: IAgentDataV2) => {
          return (
            agentId.toLowerCase().includes(query.toLowerCase()) ||
            (agentName && agentName.toLowerCase().includes(query.toLowerCase()))
          );
        };
    ...
    return query === ""
      ...
      : serverList.reduce(
          (
            acc: IServerAndAgentDataV2[],
            { groupName, instancesList }: IServerAndAgentDataV2
          ) => {
            const matchedList = instancesList.filter(filterCallback);

            return isEmpty(matchedList)
              ? acc
              : [...acc, { groupName, instancesList: matchedList }];
          },
          []
        );
  }
