...
@Component({
  ...
  templateUrl:
    './scatter-chart-for-filtered-map-side-bar-container.component.html',
  styleUrls: [
    './scatter-chart-for-filtered-map-side-bar-container.component.css',
  ],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ScatterChartForFilteredMapSideBarContainerComponent
  implements OnInit, OnDestroy
{
  ...
  private connectStore(): void {
    ...
    this.storeHelperService
      .getScatterChartData(this.unsubscribe)
      .pipe(
        filter(
          (data: any) =>
            data.length > 0 &&
            data.length > this.scatterChartDataOfAllNode.length
        )
      )
      ...
      of(null)
        ...
        .subscribe(() => {
          this.scatterChartInteractionService.addChartData(
            this.instanceKey,
            this.scatterChartDataOfAllNode[i][this.selectedApplication]
          );
        });
    }
  }
