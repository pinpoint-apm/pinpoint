...
export class InfoPerServerContainerComponent implements OnInit, OnDestroy {
              ...
              tap((histogramData = {}) => {
                ...
                this.messageQueueService.sendMessage({
                  ...
                  param: {
                    ...
                    responseSummary:
                      this.agentHistogramData["agentHistogram"][
                        this.selectedAgent
                      ],
  ...
  private getAgentName(agentId: string): string {
    ...
    for (let server of serverList) {
      const agentIds = Object.keys(
        this.agentHistogramData["serverList"][server]["instanceList"]
      );

      if (agentIds && agentIds.includes(agentId)) {
        return this.agentHistogramData["serverList"][server]["instanceList"][
          agentId
        ]["agentName"];
      }
    }
