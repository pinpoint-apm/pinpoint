...
public class RequestTraceReader {
    ...
    public Trace read(final ServerRequestWrapper serverRequestWrapper) {
        ...
        if (isDebug) {
            logger.debug("Read server request trace. serverRequestWrapper={}", serverRequestWrapper);
        }
    ...
    private boolean samplingEnable(final ServerRequestWrapper serverRequestWrapper) {
        final String samplingFlag = serverRequestWrapper.getSamplingFlag();
        if (isDebug) {
    ...
    private TraceId populateTraceIdFromRequest(final ServerRequestWrapper serverRequestWrapper) {
        final String parentApplicationNamespace = serverRequestWrapper.getParentApplicationNamespace();
        ...
        final String transactionId = serverRequestWrapper.getTransactionId();
        if (transactionId != null) {
            final long parentSpanId = serverRequestWrapper.getParentSpanId();
            final long spanId = serverRequestWrapper.getSpanId();
            final short flags = serverRequestWrapper.getFlags();
