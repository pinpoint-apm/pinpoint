 ...
 *
 * <p>One process, too. The due-rule query takes no claim, so a second instance reads the same
 * rules and evaluates them again. What that cannot do is notify twice: every event is recorded
 * under a row lock on the rule, and whether to enqueue is decided from the state read inside
 * that lock, so the second writer sees the first one's enqueue and suppresses its own. What it
 * does cost is one backend query per rule per instance, and an ordering the lock does not fix --
 * an evaluation that started earlier can take the lock later and store its older result over a
 * newer one. Running more than one instance therefore means claiming due rules, and fencing the
 * state write against a superseded evaluation; raising the replica count alone is not enough.
 */
