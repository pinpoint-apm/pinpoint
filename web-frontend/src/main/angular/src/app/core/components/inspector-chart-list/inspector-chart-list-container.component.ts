...
export class InspectorChartListContainerComponent implements OnInit {
    ...
    ngOnInit() {
        this.messageQueueService.receiveMessage(this.unsubscribe, MESSAGE_TO.INSPECTOR_CHART_MANAGER_REMOVE).subscribe(({
                                                                                                                            type,
                                                                                                                            chartName
                                                                                                                        }: { type: string, chartName: string }) => {
            if (this.type === type) {
                this.chartState = {
                    ...this.chartState,
                    [chartName]: false
        ...
        this.messageQueueService.receiveMessage(this.unsubscribe, MESSAGE_TO.INSPECTOR_CHART_MANAGER_CHANGE_ORDER).subscribe(({
                                                                                                                                  type,
                                                                                                                                  chartOrder
                                                                                                                              }: { type: string, chartOrder: string[] }) => {
            if (this.type === type) {
