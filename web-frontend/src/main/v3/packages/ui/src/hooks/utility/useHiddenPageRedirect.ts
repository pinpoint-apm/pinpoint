...
import {
  getHiddenPageRedirect,
  HiddenPageRule,
} from '@pinpoint-fe/ui/src/loader/hiddenPageRedirect';
import { useEnableServiceMap } from './useEnableServiceMap';
import { useRequestService } from './useRequestService';

/**
 * 이미 열려 있는 화면이 **지금** 감춰진 화면인지 보고, 그렇다면 옮길 목적지를 반환한다.
 * 옮길 필요가 없으면 undefined다. 어떤 화면들이 감춰지는지는 `getHiddenPageRedirect`가 안다.
 *
 * 라우트 로더(`resolveHiddenPageRedirect`)는 화면에 **들어올 때** 한 번만 판단한다. 그래서
 ...
 * (`getHiddenPageRedirect`)가 정한다. 두 갈래가 갈리면 로더는 옮기는데 화면은 안 옮기거나
 * (또는 그 반대로) 서로 되돌리는 왕복이 생긴다. 같은 이유로 `extraRules`도 로더
 * (`resolveHiddenPageRedirect`)에 넘기는 것과 **같은 목록**이어야 한다.
 *
 * **호출부는 effect가 아니라 렌더에서 이 값을 쓴다.** effect로 옮기면 한 박자 늦어, 어긋난 상태로
 * 화면이 한 번 더 그려지며 그 사이 조회가 나간다. → `InitialFetchOutlet`
 */
export const useHiddenPageRedirect = (extraRules?: HiddenPageRule[]) => {
  ...
  // (service 쪽 → servermap 쪽)에서는 serviceName을 쓰지 않으므로 값이 무엇이든 결과가 같다.
  ...
  return getHiddenPageRedirect({ pathname, search, enableServiceMap, serviceName }, extraRules);
};
