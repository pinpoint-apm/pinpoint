...
>({
  ...
  return (filterStates: FilteredMap.FilterState[]) => {
    ...
    //
    // 링크를 못 찾으면 sourceIsWas가 undefined로 남아 **도착지**가 기준이 되고 hint도 비어,
    ...
    if (!filterState?.applicationName) {
      ...
      if (link) {
        sourceIsWas = link?.sourceInfo?.nodeCategory === GetServerMap.NodeCategory.SERVER;
        addedHint =
    ...
    if (!getFilterTargetApplication(filterState, sourceIsWas)) {
      return;
    }

    window.open(
      `${BASE_PATH}${getFilteredMapPath(filterState, sourceIsWas, serviceName)}?from=${from}&to=${to}${getFilteredMapQueryString(
        {
          filterStates,
          hint: {
            currHint: parsedHint || {},
