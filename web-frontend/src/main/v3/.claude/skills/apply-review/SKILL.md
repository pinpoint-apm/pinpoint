...
gh pr view --json number,title,headRepositoryOwner,baseRepository,url
```
...
`{owner}/{repo}`는 PR이 올라간 저장소(보통 `pinpoint-apm/pinpoint`). 위 `gh pr view` 결과로 채웁니다.
...
## 4단계: 커밋 전 `/qa-pr` 필수 (게이트)

**커밋·푸시·PR 갱신 전에 반드시 `/qa-pr` 스킬을 실행하세요.**
`/qa-pr`은 빌드(`yarn build`)·테스트(`yarn test`)·동작 QA·회귀 점검을 수행합니다.
...
```bash
# 푸시 전 upstream 최신화 리베이스 (git-workflow.md)
git fetch upstream master
git rebase upstream/master

git add <변경 파일>
git commit -m "[#issue] Description"   # 영어로. 같은 플랫폼이면 #issue, 다르면 #noissue
...
gh api repos/{owner}/{repo}/pulls/{number}/comments/{comment_id}/replies \
  --method POST -f body="Applied. <쉬운 한 줄 이유>"
# 또는 무시한 경우
gh api repos/{owner}/{repo}/pulls/{number}/comments/{comment_id}/replies \
  --method POST -f body="Not applied. <쉬운 한 줄 이유>"
```

요약 리뷰(인라인이 아닌 총평)에 대응할 때는 PR 본문 코멘트를 사용:

```bash
gh pr comment {number} --body "<적용/무시 항목 요약, 영어>"
```
