...
export const InitialFetchOutlet = () => {
  ...
  // key는 해시와 **같은 값**이어야 하므로 해시가 쓰는 함수를 그대로 쓴다. 아톰은 service가
  // 바뀔 때 이 컴포넌트를 다시 렌더시키기 위해 읽는다(그 함수는 store를 명령형으로 읽는다).
  ...
  // 다른 탭에서 Experimental 설정을 바꿔, 보고 있던 화면이 메뉴에서 감춰진 쪽이 된 경우다.
  ...
  if (hiddenPageRedirect) {
    return <Navigate to={hiddenPageRedirect} replace />;
  }
