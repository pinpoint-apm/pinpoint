helix.extendRenderer('reversed_line', 'HubbleReversedLineChart', 'HubbleLineChart', function() {
        ...
        _getAllData : function(keys) {
            ...
            for (var i = 0, len = keys.length; i < len; i++) {
                ...
                for (var j = 0, len2 = tdata.length; j < len2; j++) {
                    // FIXME base value must be defined
                    var to_add = (!tdata[j] || tdata[j] > 100 || isNaN(tdata[j])) ? 0 : 100 - tdata[j];
                    allData[j] += to_add;
                }
            }
