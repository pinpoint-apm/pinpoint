...
helix.defineRenderer('filled_multi_line', 'HubbleFilledMultiLineChart', function(){
    var self = this,
        option = this.option,
        data = this.data,
    ...
    return {
        ...
        _draw : function(keys) {
            ...
            var drawData = function(key, index) {
                ...
                var area = d3.svg.area()
                    ...
                    .y0(function (d, i) {
                        var ret = top_padding + axis_height;
                        if (d != 0) {
                            ret = top_padding + axis_height + height[i] + .5
                        }
                        return ret
                    })
                    .y1(function (d, i) {
                        var ret = top_padding + axis_height;
                        if (d != 0) {
                            height[i] = yScale(d) - top_padding - axis_height
                            ret = yScale(d) + .5
                        }
                        return ret
                    })
                ...
                chart.append("path")
                    .attr("class", "area data-" + index)
                    .attr("d", area(value))
                    .style("stroke", "none")
                    .style("fill", self._getColor(index))
                    .style("fill-opacity", "0.5")
            ...
            // FIXME sort at the middle... very crude
            var kv = [];
            for (var i = 0, len = keys.length; i < len; i++) {
                var v = self.getData(keys[i], 'value')
                var middle_point = v.length / 2;
                kv.push({index:i, v:v[middle_point]});
            }

            kv.sort(function(lh, rh) {
                if (lh.v < rh.v) return -1;
                if (lh.v > rh.v) return  1;
                return 0;
            });

            kv.forEach(function(i) {
                drawData(keys[i.index], i.index);
            });
