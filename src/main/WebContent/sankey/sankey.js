d3.sankey = function () {
    ...
    sankey.link = function () {
        ...
        function link(d) {
            var x0 = d.source.x + d.source.dx, x1 = d.target.x, xi = d3
                .interpolateNumber(x0, x1), x2 = xi(curvature), x3 = xi(1 - curvature), y0 = d.source.y
                + d.sy + d.dy / 2, y1 = d.target.y + d.ty + d.dy / 2;
            return "M" + x0 + "," + y0 + "C" + x2 + "," + y0 + " " + x3 + ","
                + y1 + " " + x1 + "," + y1;
        }
    ...
    function computeNodeValues() {
        nodes.forEach(function (node) {
            node.value = Math.max(d3.sum(node.sourceLinks, value), d3.sum(
    ...
    function computeNodeDepths(iterations) {
        ...
        function relaxRightToLeft(alpha) {
            nodesByBreadth.slice().reverse().forEach(
                function (nodes) {
                    nodes.forEach(function (node) {
                        if (node.sourceLinks.length) {
                            var y = d3
        ...
        function resolveCollisions() {
            nodesByBreadth.forEach(function (nodes) {
                ...
                // If the bottommost node goes outside the bounds, push it back
                // up.
