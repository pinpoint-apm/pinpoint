...
public class ZookeeperLatestJobWorker implements Runnable {
	...
	public void run() {
		...
		// 이벤트 발생시 처리가 안될 경우 실제 등록이 안되어이는 ChannelContext가 남아있을 수 있음 이 경우 Manager에서 
	...
	public boolean handleUpdate(ChannelContext channelContext, UpdateJob job) {
	...
	public boolean handleDelete(ChannelContext channelContext, DeleteJob job) {
	...
	private void putJob(ChannelContext channelContext, Job job) {
		synchronized (lock) {
