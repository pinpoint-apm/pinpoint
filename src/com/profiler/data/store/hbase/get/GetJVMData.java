...
public class GetJVMData extends AbstractGetData {
	long dataTime=0;
	int dataPeriod=0;
	public GetJVMData(long dataTime,int dataPeriod) {
		super("JVM");
		...
		if(client!=null) {
			try {
				...
				for(ByteBuffer agent:agentList) {
					...
					List<TRowResult> jvmDataList=client.getRow(jvmTableNameBuffer,rowNameBuffer);
					//client.getRow
					...
					List<TRowResult> tpsDataList=client.getRow(tpsTableNameBuffer,rowNameBuffer);
					endTime=System.nanoTime();
					getTime=(endTime-startTime)/1000000.0;
					log("TPS getDataTime="+getTime);
					
//					List<ByteBuffer> columns=new ArrayList<ByteBuffer>();
//					client.getRowWithColumns(tableName, rowNameBuffer, columns);
//					data.append("TRowCount="+serverList.size()+"<BR>");
...
//						data.append(keyList[perColumnDataCount*9]).append(NEW_LINE);
					...
					if(tpsDataList!=null && tpsDataList.size()>0) {
						TRowResult tpsRowResult=tpsDataList.get(0);
						int columnSize=tpsRowResult.getColumnsSize();
						data.append("TPS Data Size:").append(columnSize).append(NEW_LINE);
						
						Map<ByteBuffer,TCell> columns=tpsRowResult.getColumns();
						String keyList[]=getDataKeyList(columnSize,columns);
	...
	protected void appendScatterChartValueGapData(int from,int perColumnDataCount,Map<ByteBuffer,TCell> columns,String keyList[],StringBuilder builder) {
...
//				System.out.println(gap); 
	...
	private void appendGCCountData(int perColumnDataCount,Map<ByteBuffer,TCell> columns,String keyList[],StringBuilder gcCountData) {
		String gc1Count=new String(columns.get(Converter.toByteBuffer(keyList[perColumnDataCount*2-1])).getValue());
		String gc2Count=new String(columns.get(Converter.toByteBuffer(keyList[perColumnDataCount*4-1])).getValue());
