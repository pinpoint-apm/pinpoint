...
public class DefaultProfilerPluginGlobalContext implements ProfilerPluginGlobalContext {
    ...
    public void setApplicationType(ServiceType applicationType) {
        ...
        if (this.applicationType.getCode() != applicationType.getCode()) {
            logger.info("Multiple application type detected. Overriding [{}] -> [{}]", this.applicationType.getName(), applicationType.getName());
            this.applicationType = applicationType;
        }
    }
}
