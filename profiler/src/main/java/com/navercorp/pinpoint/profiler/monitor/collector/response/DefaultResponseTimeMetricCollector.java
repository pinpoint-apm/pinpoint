...
public class DefaultResponseTimeMetricCollector implements ResponseTimeMetricCollector {
    ...
    public TResponseTime collect() {
        ResponseTimeHistogramValue responseTimeHistogram = responseTimeMetric.responseTimeHistogram();

        TResponseTime tResponseTime = new TResponseTime();

        long count = responseTimeHistogram.getCount();
        if (count != 0) {
            tResponseTime.setCount(count);
        }
        long avg = responseTimeHistogram.getAvg();
        if (avg != 0) {
