...
public class BatchApplicationIndexServiceImpl implements BatchApplicationIndexService {
    ...
    public BatchApplicationIndexServiceImpl(
            ...
            @Nullable ApplicationIndexServiceV2 applicationUidServiceV2,
            @Value("${pinpoint.batch.application.index.v2.enabled:false}") boolean v2Enabled) {
        this.applicationIndexDao = Objects.requireNonNull(applicationIndexDao, "applicationIndexDao");
        this.applicationTraceIndexDao = Objects.requireNonNull(applicationTraceIndexDao, "applicationTraceIndexDao");
        this.applicationUidServiceV2 = applicationUidServiceV2;
    ...
    public List<Application> selectAllApplications() {
        if (v2Enabled) {
            return this.applicationUidServiceV2.getApplications(ServiceUid.DEFAULT_SERVICE_UID_NAME);
        }
    ...
    public List<String> selectAllApplicationNames() {
        if (v2Enabled) {
            return this.applicationUidServiceV2.getApplications(ServiceUid.DEFAULT_SERVICE_UID_NAME).stream()
    ...
    public void remove(String applicationName) {
        ...
        if (applicationUidServiceV2 != null) {
            List<Application> applicationList = this.applicationUidServiceV2.getApplications(ServiceUid.DEFAULT_SERVICE_UID_NAME, applicationName);
            for (Application application : applicationList) {
                this.applicationUidServiceV2.deleteApplication(ServiceUid.DEFAULT_SERVICE_UID_NAME, application.getName(), application.getServiceTypeCode());
            }
        }
    }
    ...
    public List<String> selectAgentIds(String applicationName) {
        if (v2Enabled) {
            ...
            for (Application application : applicationList) {
                List<String> agentIdList = this.applicationUidServiceV2.getAgentIds(ServiceUid.DEFAULT_SERVICE_UID_NAME, application.getName(), application.getServiceTypeCode());
    ...
    private void deleteAgents(String applicationName, List<String> agentIds) {
        List<Application> applicationList = this.applicationUidServiceV2.getApplications(ServiceUid.DEFAULT_SERVICE_UID_NAME, applicationName);
        for (Application application : applicationList) {
            this.applicationUidServiceV2.deleteAgents(ServiceUid.DEFAULT_SERVICE_UID_NAME, application.getName(), application.getServiceTypeCode(), agentIds);
        }
    }
}
