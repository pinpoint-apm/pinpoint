...
public class JvmCpuDataCollector extends DataCollector implements JvmCpuDataGetter {
    ...
    public void collect() {
        ...
        for (AgentUsageCount agentUsageCount : agentUsageCountList) {
            long jvmCpuUsagedPercent = calculatePercent(agentUsageCount.getValue().longValue(), 100L * agentUsageCount.getCountValue().longValue());
            agentJvmCpuUsageRate.put(agentUsageCount.getAgentId(), jvmCpuUsagedPercent);
        }
    }
