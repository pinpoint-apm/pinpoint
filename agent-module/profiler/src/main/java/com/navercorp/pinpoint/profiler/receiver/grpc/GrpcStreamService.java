...
public class GrpcStreamService implements AutoCloseable{
    ...
    private final int maxActiveTraceCount = 4;
    ...
    private void checkLimit() {
        if (grpcProfilerStreamSocketList.size() >= maxActiveTraceCount) {
            final ActiveThreadCountStreamSocket remove = grpcProfilerStreamSocketList.remove(0);
            if (remove != null) {
                remove.close();
            }
        }
