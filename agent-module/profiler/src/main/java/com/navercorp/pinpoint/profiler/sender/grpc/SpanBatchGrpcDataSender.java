...
public class SpanBatchGrpcDataSender extends AbstractGrpcDataSender<SpanType> {
    ...
    private void sendLoop() {
        while (!shutdown) {
            try {
                ...
                if (CollectionUtils.hasLength(batch)) {
                    sendBatch(batch);
                }
    ...
    private void sendBatch(List<SpanType> batch) {
        final PSpanMessageBatch spanMessageList = toBatchMessage(batch);
    ...
    private PSpanMessageBatch toBatchMessage(List<SpanType> batch) {
        if (batch.isEmpty()) {
