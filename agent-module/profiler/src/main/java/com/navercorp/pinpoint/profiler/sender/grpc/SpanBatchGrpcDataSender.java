...
public class SpanBatchGrpcDataSender extends AbstractGrpcDataSender<SpanType> {
    ...
    private void sendLoop() {
        while (!shutdown) {
            try {
                final List<SpanType> batch = collectBatch();
                if (!batch.isEmpty()) {
                    sendBatch(batch);
                }
    ...
    private List<SpanType> collectBatch() throws InterruptedException {
        final List<SpanType> batch = new ArrayList<>(batchSize);
    ...
    private void sendBatch(List<SpanType> batch) {
        final PSpanMessageBatch.Builder listBuilder = PSpanMessageBatch.newBuilder();
        for (SpanType spanType : batch) {
            ...
            if (spanMessage != null) {
                listBuilder.addSpan(spanMessage);
            }
        }
        final PSpanMessageBatch spanMessageList = listBuilder.build();
        ...
        spanStub.sendSpanList(spanMessageList, new StreamObserver<PSpanResultBatch>() {
            @Override
            public void onNext(PSpanResultBatch response) {
                if (response.hasPartialSuccess()) {
                    ...
                    if (rejectedSpans > 0) {
                        logger.warn("sendSpanList partial success: rejectedSpans={}, errorMessage={}",
                                rejectedSpans, partialSuccess.getErrorMessage());
                    } else if (!partialSuccess.getErrorMessage().isEmpty()) {
                        logger.info("sendSpanList warning: {}", partialSuccess.getErrorMessage());
                    }
                }
            }
    ...
    private PSpanMessage toSpanMessage(GeneratedMessageV3 message) {
        if (message instanceof PSpan) {
            ...
            return PSpanMessage.newBuilder().setSpan(pSpan).build();
        }
        if (message instanceof PSpanChunk) {
            ...
            return PSpanMessage.newBuilder().setSpanChunk(pSpanChunk).build();
        }
