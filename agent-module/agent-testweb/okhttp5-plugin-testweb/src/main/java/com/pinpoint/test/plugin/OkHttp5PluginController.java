...
public class OkHttp5PluginController {
    ...
    @GetMapping("/client/enqueue")
    public Mono<String> clientEnqueue() {
        return enqueue("http://naver.com");
    }

    @GetMapping("/client/enqueue-local")
    public Mono<String> clientEnqueueLocal() {
        return enqueue("http://localhost:18080");
    }

    @GetMapping("/client/enqueue-unknown")
    public Mono<String> clientEnqueueUnknown() {
        return enqueue("http://falfjlajdflajflajlf");
    }

    @GetMapping("/client/cancel")
    public Mono<String> clientCancel() {
        OkHttpClient httpClient = new OkHttpClient();
        Request request = new Request.Builder().url("http://naver.com").build();
        return Mono.create(sink -> {
            ...
            call.enqueue(new Callback() {
                ...
                public void onFailure(Call call, IOException e) {
                    sink.success("canceled: " + e.getMessage());
                }

                @Override
                public void onResponse(Call call, Response response) {
                    sink.success("completed before cancel: " + response.message());
                }
            });
            call.cancel();
        });
    ...
    private Mono<String> enqueue(String url) {
        OkHttpClient httpClient = new OkHttpClient();
        Request request = new Request.Builder().url(url).build();
        return Mono.create(sink -> httpClient.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                sink.error(e);
            }
