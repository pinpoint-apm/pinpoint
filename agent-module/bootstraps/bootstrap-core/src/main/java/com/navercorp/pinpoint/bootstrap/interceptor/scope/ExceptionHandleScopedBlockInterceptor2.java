...
public class ExceptionHandleScopedBlockInterceptor2 implements BlockAroundInterceptor2 {
    private final PluginLogger logger = PluginLogManager.getLogger(getClass());
    private final boolean debugEnabled = logger.isDebugEnabled();

    private final BlockAroundInterceptor2 interceptor;
    private final InterceptorScope scope;
    private final ExecutionPolicy policy;
    private final ExceptionHandler exceptionHandler;

    public ExceptionHandleScopedBlockInterceptor2(BlockAroundInterceptor2 interceptor, InterceptorScope scope, ExecutionPolicy policy, ExceptionHandler exceptionHandler) {
    ...
    @Override
    public TraceBlock before(Object target, Object arg0, Object arg1) {
        final InterceptorScopeInvocation transaction = scope.getCurrentInvocation();

        if (transaction.tryEnter(policy)) {
            try {
                return interceptor.before(target, arg0, arg1);
            } catch (Throwable t) {
    ...
    @Override
    public void after(TraceBlock block, Object target, Object arg0, Object arg1, Object result, Throwable throwable) {
        final InterceptorScopeInvocation transaction = scope.getCurrentInvocation();

        if (transaction.canLeave(policy)) {
            try {
                interceptor.after(block, target, arg0, arg1, result, throwable);
            } catch (Throwable t) {
                exceptionHandler.handleException(t);
            } finally {
