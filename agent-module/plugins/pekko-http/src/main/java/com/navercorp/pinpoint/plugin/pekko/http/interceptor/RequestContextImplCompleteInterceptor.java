            ...
            if (result instanceof Future && ((Future) result).isCompleted()) {
                ...
                if (routeResult instanceof Success) {
                    ...
                    if (success instanceof Complete) {
                        org.apache.pekko.http.javadsl.model.HttpResponse response = ((Complete) success).getResponse();
                        if (response == null) {
                            return;
                        }
                        org.apache.pekko.http.javadsl.model.StatusCode status = response.status();
                        if (status == null) {
                            return;
                        }
