...
public class ParameterRecorderFactory {
    public static ParameterRecorder<HttpRequestAndContext> newParameterRecorderFactory(Filter<String> excludeProfileMethodFilter, boolean traceRequestParam) {
        if (!traceRequestParam) {
            return new DisableParameterRecorder<>();
        }
        ParameterExtractor<HttpRequestAndContext> parameterExtractor = new HttpRequestParameterExtractor(64, 512);
        ParameterExtractor<HttpRequestAndContext> methodFilterExtractor = new MethodFilterExtractor(excludeProfileMethodFilter, parameterExtractor);
