import { Component, OnInit, OnDestroy, OnChanges, Input, Output, EventEmitter, ViewChild, ElementRef, SimpleChanges, AfterViewInit, ChangeDetectorRef } from '@angular/core';
...
import { ScatterChartInteractionService, IRangeParam, IResetParam, IChangedAgentParam } from './scatter-chart-interaction.service';
...
export class ScatterChartV2Component implements OnInit, OnDestroy, OnChanges, AfterViewInit {
    ...
    getDataByRange(x1: number, x2: number, y1: number, y2: number, agentId: string, dotStatus: string[]): [string, number, number][] {
        ...
        return dotList
            .filter(({x, y, type}: IScatterDataType) => (x1 <= x && x <= x2) && (y1 <= y && y <= y2) && dotStatus.includes(type))
