...
export class ApplicationUnloadedClassCountChartContainer implements IInspectorChartContainer {
    ...
    makeChartData({charts}: IInspectorChartData): PrimitiveArray[] {
        this.minAgentIdList = makeYData(charts.y['UNLOADED_CLASS_COUNT'], 1) as string[];
        this.maxAgentIdList = makeYData(charts.y['UNLOADED_CLASS_COUNT'], 3) as string[];

        return [
            ['x', ...makeXData(charts.x)],
            ['max', ...makeYData(charts.y['UNLOADED_CLASS_COUNT'], 2)],
            ['avg', ...makeYData(charts.y['UNLOADED_CLASS_COUNT'], 4)],
            ['min', ...makeYData(charts.y['UNLOADED_CLASS_COUNT'], 0)],
        ];
    }
