...
export class ApplicationLoadedClassCountChartContainer implements IInspectorChartContainer {
    ...
    makeChartData({charts}: IInspectorChartData): PrimitiveArray[] {
        this.minAgentIdList = makeYData(charts.y['LOADED_CLASS_COUNT'], 1) as string[];
        this.maxAgentIdList = makeYData(charts.y['LOADED_CLASS_COUNT'], 3) as string[];

        return [
            ['x', ...makeXData(charts.x)],
            ['max', ...makeYData(charts.y['LOADED_CLASS_COUNT'], 2)],
            ['avg', ...makeYData(charts.y['LOADED_CLASS_COUNT'], 4)],
            ['min', ...makeYData(charts.y['LOADED_CLASS_COUNT'], 0)],
        ];
    }
