...
public class JvmGcChart implements StatChart {
    ...
    public static class JvmGcChartGroup implements StatChartGroup {
        ...
        private Chart<AgentStatPoint<Long>> newChart(List<SampledJvmGc> jvmGcList, Function<SampledJvmGc, AgentStatPoint<Long>> filter) {
            List<AgentStatPoint<Long>> filterList = filter(jvmGcList, filter);
            TimeSeriesChartBuilder<AgentStatPoint<Long>> builder = new TimeSeriesChartBuilder<>(this.timeWindow, SampledJvmGc.UNCOLLECTED_POINT_CREATER);
            return builder.build(filterList);
        }


        private List<AgentStatPoint<Long>> filter(List<SampledJvmGc> jvmGcList, Function<SampledJvmGc, AgentStatPoint<Long>> filter) {
            List<AgentStatPoint<Long>> result = new ArrayList<>(jvmGcList.size());
            for (SampledJvmGc jvmGc : jvmGcList) {
                AgentStatPoint<Long> apply = filter.apply(jvmGc);
                result.add(apply);
            }
            return result;
        }
