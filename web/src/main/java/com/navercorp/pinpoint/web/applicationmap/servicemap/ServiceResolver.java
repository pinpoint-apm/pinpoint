...
public class ServiceResolver {
    ...
    private static final ServiceMappingRule[] EMPTY_RULES = new ServiceMappingRule[0];
    private static final ServiceResolver EMPTY = new ServiceResolver(EMPTY_RULES);

    private final ServiceMappingRule[] rules;

    public ServiceResolver(List<ServiceMappingRule> rules) {
        Objects.requireNonNull(rules, "rules");
        this.rules = rules.toArray(EMPTY_RULES);
    }

    private ServiceResolver(ServiceMappingRule[] rules) {
        this.rules = rules;
    }
