...
import com.navercorp.pinpoint.web.dao.ApplicationTraceIndexV2Dao;
import com.navercorp.pinpoint.web.mapper.TraceIndexMetaScatterMapperV2;
...
@Repository
public class HbaseApplicationTraceIndexV2Dao implements ApplicationTraceIndexV2Dao {

    private static final int APPLICATION_TRACE_INDEX_V2_NUM_PARTITIONS = 8;
    ...
    public HbaseApplicationTraceIndexV2Dao(ScatterChartProperties scatterChartProperties,
                                           HbaseOperations hbaseOperations,
                                           TableNameProvider tableNameProvider,
                                           @Qualifier("transactionIdMapperV2") RowMapper<List<TransactionId>> traceIndexMapperV2,
                                           @Qualifier("traceIndexScatterMapperV2") RowMapper<List<Dot>> traceIndexScatterMapperV2,
    ...
    public boolean hasTraceIndex(int serviceUid, String applicationName, int serviceTypeCode, Range range, boolean backwardDirection) {
        ...
        TableName applicationTraceIndexTableName = tableNameProvider.getTableName(INDEX_V2.getTable());
        List<List<TransactionId>> traceIndexList = hbaseOperations.findParallel(applicationTraceIndexTableName,
                scan, traceIndexDistributor, 1, traceIndexMapperV2, lastRowAccessor, APPLICATION_TRACE_INDEX_V2_NUM_PARTITIONS);
    ...
    public LimitedScanResult<List<TransactionId>> scanTraceIndex(int serviceUid, final String applicationName, int serviceTypeCode, Range range, int limit, boolean scanBackward) {
        ...
        TableName applicationTraceIndexTableName = tableNameProvider.getTableName(INDEX_V2.getTable());
        List<List<TransactionId>> traceIndexList = hbaseOperations.findParallel(applicationTraceIndexTableName,
                scan, traceIndexDistributor, limit, traceIndexMapperV2, lastRowAccessor, APPLICATION_TRACE_INDEX_V2_NUM_PARTITIONS);
    ...
    private Scan createScan(int serviceUid, String applicationName, int serviceTypeCode, Range range, boolean scanBackward, int limit) {
        ...
        scan.addFamily(INDEX_V2.getName());
        scan.setId("ApplicationTraceIndexV2Scan");
    ...
    public LimitedScanResult<List<Dot>> scanTraceScatterData(int serviceUid, String applicationName, int serviceTypeCode, Range range, int limit, boolean scanBackward) {
        ...
        List<List<Dot>> listList = hbaseOperations.findParallel(applicationTraceIndexTableName, scan,
                traceIndexDistributor, limit, this.traceIndexScatterMapperV2, APPLICATION_TRACE_INDEX_V2_NUM_PARTITIONS);
    ...
    public LimitedScanResult<List<DotMetaData>> scanScatterDataV2(int serviceUid, String applicationName, int serviceTypeCode, DragAreaQuery dragAreaQuery, int limit) {
        ...
        RowMapper<List<DotMetaData>> mapper = new TraceIndexMetaScatterMapperV2(filter);
    ...
    private <R> LimitedScanResult<List<R>> scanScatterData0(int serviceUid, String applicationName, int serviceTypeCode, DragAreaQuery dragAreaQuery, int limit,
        ...
        if (metadataScan) {
            scan.addFamily(META_V2.getName());
        }

        TableName applicationTraceIndexTableName = tableNameProvider.getTableName(INDEX_V2.getTable());
        List<List<R>> dotListList = hbaseOperations.findParallel(applicationTraceIndexTableName, scan,
                traceIndexDistributor, limit, mapper, lastRowAccessor, APPLICATION_TRACE_INDEX_V2_NUM_PARTITIONS);
    ...
    private Filter newFuzzyFilter(DragArea dragArea) {
        ...
        return this.fuzzyRowKeyBuilder.buildV2(yHigh, yLow);
    }
    ...
    private class LastRowAccessor implements LimitEventHandler {
        ...
        public void handleLastResult(Result lastResult) {
            ...
            final Cell last = CellUtils.lastCell(lastResult.rawCells(), INDEX_V2.getName());
            this.lastRowTimestamp = TraceIndexRowUtils.extractTimestamp(last);
