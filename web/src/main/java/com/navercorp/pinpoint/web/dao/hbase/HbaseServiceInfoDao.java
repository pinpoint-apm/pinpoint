...
public class HbaseServiceInfoDao implements ServiceInfoDao {
    ...
    private static final HbaseColumnFamily.ServiceApplicationIndex SERVICE_APPLICATION_INDEX = HbaseColumnFamily.SERVICE_APPLICATION_INDEX;
    ...
    public List<ApplicationId> getApplicationIds(ServiceId serviceId) {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("getApplicationIds() serviceId:{}", serviceId);
        }

        TableName tableName = this.tableNameProvider.getTableName(SERVICE_APPLICATION_INDEX.getTable());
        byte[] rowKey = serviceId.toBytes();
        byte[] family = SERVICE_APPLICATION_INDEX.getName();
