...
public class ResponseTimeV3ResultExtractor implements ResultsExtractor<List<ResponseTime>> {
    ...
    public ResponseTimeV3ResultExtractor(ServiceTypeRegistryService registry,
    ...
    private ResponseTime.Builder createResponseTimeBuilder(Map<Key, ResponseTime.Builder> map, UidLinkRowKey uidRowKey) {
        final long timestamp = timeWindowFunction.refineTimestamp(TimeUtils.recoveryTimeMillis(uidRowKey.getTimestamp()));
        final ServiceType serviceType = registry.findServiceType(uidRowKey.getServiceType());

        final Key key = new Key(uidRowKey.getServiceUid(), uidRowKey.getApplicationName(), uidRowKey.getServiceType(), timestamp);

        return map.computeIfAbsent(key, k -> ResponseTime.newBuilder(uidRowKey.getApplicationName(), serviceType, timestamp));
    }
