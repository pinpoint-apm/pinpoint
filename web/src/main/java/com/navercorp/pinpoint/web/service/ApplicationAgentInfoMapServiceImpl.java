...
public class ApplicationAgentInfoMapServiceImpl implements ApplicationAgentInfoMapService {
    ...
    @Override
    public AgentsMapByHost getAgentsListByApplicationName(String applicationName,
                                                          Range range,
        ...
        List<AgentAndStatus> agentInfoAndStatuses = getActivateAgentInfoList(applicationName, range, applicationAgentListQueryRule, agentInfoPredicate);
    ...
    private List<AgentAndStatus> getActivateAgentInfoList(String applicationName,
                                                          Range range,
                                                          ApplicationAgentListQueryRule applicationAgentListQueryRule,
                                                          AgentInfoFilter agentInfoPredicate) {
        return switch (applicationAgentListQueryRule) {
            case ACTIVE_STATUS -> applicationAgentListService.activeStatusAgentList(applicationName, range, agentInfoPredicate);
            case ACTIVE_RESPONSE -> applicationAgentListService.activeResponseAgentList(applicationName, range, agentInfoPredicate);
            default -> applicationAgentListService.allAgentList(applicationName, range, agentInfoPredicate);
        };
    }
}
