...
@Beta
@Component
public class PassiveSpanMapperV2 implements RowMapper<List<PassiveSpanBo>> {
    ...
    @Autowired
    public PassiveSpanMapperV2(@Qualifier("traceRowKeyDecoderV2") RowKeyDecoder<TransactionId> rowKeyDecoder) {
    ...
    @Override
    public List<PassiveSpanBo> mapRow(Result result, int rowNum) throws Exception {
        if (result.isEmpty()) {
        ...
        for (Cell cell : rawCells) {
            ...
            if (CellUtil.matchingFamily(cell, HBaseTables.TRACE_V2_CF_PASSIVE_SPAN)) {
                ...
                if (decodeObject instanceof PassiveSpanBo) {
                    PassiveSpanBo passiveSpanBo = (PassiveSpanBo) decodeObject;
                    if (logger.isDebugEnabled()) {
                        logger.debug("passiveSpanBo:{}", passiveSpanBo);
                    }
