(function() {
	 ...
	 * @class
	 */
	pinpointApp.constant("scatterDirective2Config", {
	...
	pinpointApp.directive("scatterDirective2", ["scatterDirective2Config", "$rootScope", "$compile", "$timeout", "webStorage", "$window", "TooltipService", "AnalyticsService", "CONST_SET",
				...
				link: function (scope, element, attrs) {
					...
					function makeScatter(target, application, w, h, scatterData) {
						...
						var oScatterChart = new BigScatterChart2(options, getAgentList(scatterData), [
							...
							new BigScatterChart2.DownloadFeature( cfg.images.download, "PNG" ).addCallback( function( oChart ) {
								analyticsService.send( analyticsService.CONST.MAIN, analyticsService.CONST.CLK_DOWNLOAD_SCATTER );
							}),
							new BigScatterChart2.WideOpenFeature( cfg.images.fullscreen ).addCallback( function() {
							...
							new BigScatterChart2.HelpFeature( tooltipService )
						], {
					...
					scope.$on("scatterDirective2.initialize", function (event, navbarVoService) {
						console.log("----------init");
						oNavbarVoService = navbarVoService;
						initScatterHash();
						element.empty();
					});
					scope.$on("scatterDirective2.initializeWithNode", function (event, node, w, h) {
						console.log("----------WithNode");
						scope.currentAgent = CONST_SET.AGENT_ALL;
						htLastNode = node;
						showScatter(node.key, w, h);
					});
					scope.$on("scatterDirective2.initializeWithData", function (event, application, data) {
					...
					scope.$on("scatterDirective2.showByNode", function (event, node) {
						htLastNode = node;
