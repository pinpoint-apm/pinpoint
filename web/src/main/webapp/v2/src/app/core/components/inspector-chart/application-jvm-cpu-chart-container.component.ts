...
@Component({
    selector: 'pp-application-jvm-cpu-chart-container',
    templateUrl: './application-jvm-cpu-chart-container.component.html',
    styleUrls: ['./application-jvm-cpu-chart-container.component.css'],
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ApplicationJVMCPUChartContainerComponent extends InspectorChartContainer implements OnInit, OnDestroy {
    constructor(
    ...
    protected makeChartData(chartData: IChartDataFromServer): {[key: string]: any} {
        ...
        const cpuLoadJVM = chartData.charts.y['CPU_LOAD_JVM'];
        ...
        for ( let i = 0 ; i < dataCount ; i++ ) {
            ...
            if (cpuLoadJVM[i]) {
                minArr.push(this.parseData(cpuLoadJVM[i][0]));
                minAgentIdArr.push(cpuLoadJVM[i][1]);
                maxArr.push(this.parseData(cpuLoadJVM[i][2]));
                maxAgentIdArr.push(cpuLoadJVM[i][3]);
                avgArr.push(this.parseData(cpuLoadJVM[i][4]));
            }
        }
        return {
            x: xArr,
    ...
    onShowHelp($event: MouseEvent): void {
        super.onShowHelp($event, HELP_VIEWER_LIST.APPLICATION_JVM_CPU_USAGE);
    }
}
