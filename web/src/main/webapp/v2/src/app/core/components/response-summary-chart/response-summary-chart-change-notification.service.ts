...
export interface IResponseSummaryChartNotificationData {
    chart: {
        keys: string[];
        max: number;
        values: number[];
...
@Injectable()
export class ResponseSummaryChartChangeNotificationService {
    private unsubscribe: Subject<null> = new Subject();
    private outMainSideBarChartData: Subject<IResponseSummaryChartNotificationData> = new Subject();
    private outFilteredMapSideBarChartData: Subject<any> = new Subject();
    private outInfoPerServerChartData: Subject<IResponseSummaryChartNotificationData> = new Subject();
    ...
    private connectStore(): void {
        ...
        this.storeHelperService.getServerMapTargetSelectedByList(this.unsubscribe).subscribe((target: any) => {
            ...
            const oData = {
                chart: this.agentHistogramDataService.makeChartDataForResponseSummary(
                    target.histogram,
                    this.getChartYMax()
                ),
                error: false,
    ...
    private loadChartData(from?: number, to?: number): void {
        ...
        if (target) {
            if (this.isAllAgent() && arguments.length !== 2) {
                ...
                if (target['agentHistogram']) {
                    this.outFilteredMapSideBarChartData.next({
                        chart: this.agentHistogramDataService.makeChartDataForResponseSummary(
                            target['agentHistogram'][this.selectedAgent],
                            this.getChartYMax()
                        ),
                        error: false,
                        hidden: false
