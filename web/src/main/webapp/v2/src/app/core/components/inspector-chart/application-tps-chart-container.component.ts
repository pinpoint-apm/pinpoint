...
@Component({
    selector: 'pp-application-tps-chart-container',
    templateUrl: './application-tps-chart-container.component.html',
    styleUrls: ['./application-tps-chart-container.component.css'],
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ApplicationTPSChartContainerComponent extends InspectorChartContainer implements OnInit, OnDestroy {
    constructor(
        ...
        chartDataService: ApplicationTPSChartDataService,
    ...
    protected makeChartData(chartData: IChartDataFromServer): {[key: string]: any} {
        ...
        const transactionCount = chartData.charts.y['TRANSACTION_COUNT'];
        ...
        for (let i = 0; i < dataCount; i++) {
            ...
            if (transactionCount[i]) {
                minArr.push(this.parseData(transactionCount[i][0]));
                minAgentIdArr.push(transactionCount[i][1]);
                maxArr.push(this.parseData(transactionCount[i][2]));
                maxAgentIdArr.push(transactionCount[i][3]);
                avgArr.push(this.parseData(transactionCount[i][4]));
            }
        }
        return {
            x: xArr,
            ...
            tooltips: {
                ...
                callbacks: {
                    ...
                    label: (value: {[key: string]: any}, d: {[key: string]: any}): string => {
                        ...
                        return `${label}: ${isNaN(value.yLabel) ? `-` : value.yLabel.toFixed(1) + ` ` + this.getAgentId(data.minAgentIdArr, data.maxAgentIdArr, label, index)}`;
                    }
                }
            },
            hover: {
    ...
    private convertWithUnit(value: number): string {
        ...
        result = Number.isInteger(result) ? result : Number(result.toFixed(1));
    ...
    onShowHelp($event: MouseEvent): void {
        super.onShowHelp($event, HELP_VIEWER_LIST.APPLICATION_TPS);
    }
}
