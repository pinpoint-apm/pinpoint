...
export class ApplicationJVMHeapChartContainerComponent extends InspectorChartContainer implements OnInit, OnDestroy {
    ...
    protected makeChartData(chartData: IChartDataFromServer): {[key: string]: any} {
        ...
        const memoryHeap = chartData.charts.y['MEMORY_HEAP'];
        ...
        for ( let i = 0 ; i < dataCount ; i++ ) {
            ...
            if (memoryHeap[i]) {
                minArr.push(this.parseData(memoryHeap[i][0]));
                minAgentIdArr.push(memoryHeap[i][1]);
                maxArr.push(this.parseData(memoryHeap[i][2]));
                maxAgentIdArr.push(memoryHeap[i][3]);
                avgArr.push(this.parseData(memoryHeap[i][4]));
            }
        }
    ...
    onShowHelp($event: MouseEvent): void {
        super.onShowHelp($event, HELP_VIEWER_LIST.APPLICATION_HEAP);
    }
}
