...
public class HbaseTraceIndexDaoTest {
    ...
    public void scanTraceIndexMetaTest() {
        final List<DotMetaData> scannedList = Collections.nCopies(10, testDotMetaData);
    ...
    public void scanTraceIndexDotTest() {
        List<Dot> scatterDotList = createScatterDotList();
        when(this.hbaseOperations.findParallel(any(TableName.class), any(Scan.class), any(RowKeyDistributor.class),
    ...
    private List<Dot> createScatterDotList() {
        List<Dot> result = new ArrayList<>();
        TransactionId transactionId = TransactionId.of("A", 1, 1);
        result.add(new Dot(transactionId, 2000L, 1000, 0, "a1"));
        result.add(new Dot(transactionId, 3000L, 5000, 0, "a2"));
        result.add(new Dot(transactionId, 2400L, 3000, 0, "a3"));
        return result;
    }
