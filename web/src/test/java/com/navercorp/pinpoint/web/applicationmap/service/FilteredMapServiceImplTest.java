...
public class FilteredMapServiceImplTest {
    ...
    public void init() {
        ...
        filteredMapService = new FilteredMapServiceImpl(traceDao, registry, applicationFactory, serverInstanceDatasourceService, Optional.empty(), applicationMapBuilderFactory);
    ...
    public void twoTier() {
        ...
        for (Node node : nodes) {
            ...
            if (application.getName().equals(UserNodeUtils.newUserNodeName("ROOT_APP", ServiceType.TEST_STAND_ALONE)) && application.getServiceType().getCode() == TestTraceUtils.USER_TYPE_CODE) {
                ...
                List<TimeHistogramViewModel> applicationTimeHistogram = getApplicationTimeHistogram(nodeHistogram);
...
//                AgentResponseTimeViewModelList agentTimeHistogram = nodeHistogram.getAgentTimeHistogram(TimeHistogramFormat.V1);
        ...
        for (Link link : links) {
            ...
            if ((fromApplication.getName().equals(UserNodeUtils.newUserNodeName("ROOT_APP", ServiceType.TEST_STAND_ALONE)) && fromApplication.getServiceType().getCode() == TestTraceUtils.USER_TYPE_CODE) &&
                ...
                // time histogram
                List<TimeHistogramViewModel> histogramViewModels = getTimeHistogramViewModels(link);
                ...
                assertHistogram(histogram, 1, 0, 0, 0, 0);
                // time histogram
                List<TimeHistogramViewModel> linkApplicationTimeSeriesHistogram = getTimeHistogramViewModels(link);
                HistogramSchema targetHistogramSchema = toApplication.getServiceType().getHistogramSchema();
                List<TimeCount> expectedTimeCounts = List.of(
                        new TimeCount(timeWindow.refineTimestamp(appASpanCollectorAcceptTime), 1)
                );
