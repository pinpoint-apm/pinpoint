...
import com.navercorp.pinpoint.collector.dao.hbase.statistics.CalleeCompactColumnName;
...
public class HbaseMapStatisticsCallerCompactDao implements MapStatisticsCallerCompactDao {
    ...
    @Autowired
    public HbaseMapStatisticsCallerCompactDao(MapLinkConfiguration mapLinkConfiguration,
                                              AcceptedTimeService acceptedTimeService,
                                              TimeSlot timeSlot,
                                              @Qualifier("callerCompactBulkWriter") BulkWriter bulkWriter) {
    ...
    public void update(String callerApplicationName, ServiceType callerServiceType, String calleeApplicationName, ServiceType calleeServiceType, String calleeHost, int elapsed, boolean isError) {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("[Caller] {} ({}) -> {} ({})[{}]", callerApplicationName, callerServiceType,
        ...
        final ColumnName calleeColumnName = new CalleeCompactColumnName(calleeServiceType.getCode(), calleeApplicationName, host, slotNumber);
        this.bulkWriter.increment(callerRowKey, calleeColumnName);

        if (mapLinkConfiguration.isEnableAvg()) {
            final ColumnName sumColumnName = new CalleeCompactColumnName(calleeServiceType.getCode(), calleeApplicationName, host, histogramSchema.getSumStatSlot().getSlotTime());
            this.bulkWriter.increment(callerRowKey, sumColumnName, elapsed);
        }
        if (mapLinkConfiguration.isEnableMax()) {
            final ColumnName maxColumnName = new CalleeCompactColumnName(calleeServiceType.getCode(), calleeApplicationName, host, histogramSchema.getMaxStatSlot().getSlotTime());
