...
public class GrpcSpanReceiverConfiguration {
    ...
    public ServerServiceDefinition spanServerServiceDefinition(SimpleHandler<PSpan> spanHandler,
        ...
        BindableService spanService = new SpanService(new UidGuardedHandler<>(spanHandler), new UidGuardedHandler<>(spanCheckHandler),
    ...
    @Bean
    @Qualifier("spanInterceptor")
    public ServerInterceptor spanInterceptorList(ObjectNameVersion version) {
        ServerInterceptorFactory serverInterceptorFactory = new ServerInterceptorFactory("span");
