...
public class HbaseServiceInfoDao implements ServiceInfoDao {
    ...
    private static final HbaseColumnFamily.ServiceApplicationIndex DESCRIPTOR_INDEX = HbaseColumnFamily.SERVICE_APPLICATION_INDEX;

    private final HbaseOperations hbaseTemplate;
    private final TableNameProvider tableNameProvider;
    private final RowMapper<ServiceInfo> forwardRowMapper;
    private final RowMapper<ServiceId> inverseRowMapper;
    private final RowMapper<List<ApplicationId>> serviceIndexMapper;
    ...
    public HbaseServiceInfoDao(
            ...
            @Qualifier("serviceIndexMapper") RowMapper<List<ApplicationId>> serviceIndexMapper) {
        ...
        this.serviceIndexMapper = Objects.requireNonNull(serviceIndexMapper, "serviceIndexMapper");
    }
    ...
    public List<ApplicationId> getApplications(ServiceId serviceId) {
        ...
        TableName tableName = this.tableNameProvider.getTableName(DESCRIPTOR_INDEX.getTable());
        byte[] rowKey = serviceId.toBytes();
        byte[] family = DESCRIPTOR_INDEX.getName();

        Get get = new Get(rowKey);
        get.addFamily(family);

        return hbaseTemplate.get(tableName, get, this.serviceIndexMapper);
    }

    @Override
    public void insertApplicationIntoService(ServiceId serviceId, ApplicationId applicationId) {
        Objects.requireNonNull(serviceId, "serviceId");
        Objects.requireNonNull(applicationId, "applicationId");

        if (logger.isDebugEnabled()) {
            logger.debug("insertApplicationIntoService() serviceId:{}, applicationId:{}", serviceId, applicationId);
        }

        TableName tableName = this.tableNameProvider.getTableName(DESCRIPTOR_INDEX.getTable());
        byte[] rowKey = serviceId.toBytes();
        byte[] family = DESCRIPTOR_INDEX.getName();
        byte[] qualifier = applicationId.toBytes();
