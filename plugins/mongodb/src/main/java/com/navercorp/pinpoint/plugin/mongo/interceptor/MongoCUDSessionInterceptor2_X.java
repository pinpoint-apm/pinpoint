 ...
 * @author Community
 */
public class MongoCUDSessionInterceptor2_X extends SpanEventSimpleAroundInterceptorForPlugin {
    private final boolean collectJson;
    private final boolean traceBsonBindValue;

    public MongoCUDSessionInterceptor2_X(TraceContext traceContext, MethodDescriptor descriptor, boolean collectJson,
    ...
    protected void doInBeforeTrace(SpanEventRecorder recorder, Object target, Object[] args) {
        ...
        String writeConcern = ((MongoDatabaseInfo) databaseInfo).getWriteConcern();
        if (writeConcern == null || writeConcern.trim().equals("")) {
            DBCollection collection = (DBCollection) target;
            writeConcern = collection.getWriteConcern().getWString();
        }
        MongoUtil.recordMongoCollection(recorder, ((MongoDatabaseInfo) databaseInfo).getCollectionName(), writeConcern);
    }
