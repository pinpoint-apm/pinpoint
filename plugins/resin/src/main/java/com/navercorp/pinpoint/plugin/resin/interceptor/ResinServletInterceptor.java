...
public class ResinServletInterceptor implements AroundInterceptor {
    ...
    private final boolean isResinTraceRequestParam;
    private final boolean isResinTraceCookies;
    ...
    public ResinServletInterceptor(TraceContext traceContext, MethodDescriptor descriptor) {
        ...
        this.isResinTraceRequestParam = resinConfig.isResinTraceRequestParam();
        this.isResinTraceCookies = resinConfig.isResinTraceCookies();
    ...
    public void after(Object target, Object[] args, Object result, Throwable throwable) {
        ...
        try {
            ...
            if (this.isResinTraceRequestParam) {
                logger.info("resin param start");
                final HttpServletRequest request = (HttpServletRequest) args[0];
            ...
            if (this.isResinTraceCookies) {
                final HttpServletRequest request = (HttpServletRequest) args[0];
                final String cookies = getCookie(request, 64, 512);
    ...
    private String getCookie(HttpServletRequest request, int eachLimit, int totalLimit) {
        Cookie[] cookies = request.getCookies();
        final StringBuilder cookieString = new StringBuilder(64);
        if (null != cookies) {
            for (Cookie cookie : cookies) {
                if (cookieString.length() > totalLimit) {
                    cookieString.append("...");
                    return cookieString.toString();
                }
                cookieString.append(StringUtils.abbreviate(cookie.getName(), eachLimit));
                cookieString.append("=");
                cookieString.append(StringUtils.abbreviate(cookie.getValue(), eachLimit));
                cookieString.append(",");
            }
        }
        return cookieString.toString();
