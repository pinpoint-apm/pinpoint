...
public class PostgresqlConnectionFactoryConstructorInterceptor extends SetDatabaseInfoInterceptor {
    @Override
    public DatabaseInfo getDatabaseInfo(Object target, Object[] args, Object result) {
        DatabaseInfoAccessor databaseInfoAccessor = ArrayArgumentUtils.getArgument(args, 0, DatabaseInfoAccessor.class);
        if (databaseInfoAccessor == null) {
            databaseInfoAccessor = ArrayArgumentUtils.getArgument(args, 1, DatabaseInfoAccessor.class);
        }
        if (databaseInfoAccessor == null) {
            return null;
        }
