...
public class PostgreSqlJdbcUrlParser implements JdbcUrlParser {
    ...
    private static final String JDBC_POSTGRESQL_LOADBALANCE = JDBC_URL_PREFIX + "loadbalance:";

    @Override
    public JdbcUrlParsingResult parse(String url) {
        if (url == null) {
            return new JdbcUrlParsingResult(false, UnKnownDatabaseInfo.createUnknownDataBase(PostgreSqlConstants.POSTGRESQL, PostgreSqlConstants.POSTGRESQL_EXECUTE_QUERY, null));
        }
    ...
    private JdbcUrlParsingResult parseLoadbalancedUrl(String url) {
        ...
        DatabaseInfo databaseInfo = new DefaultDatabaseInfo(PostgreSqlConstants.POSTGRESQL, PostgreSqlConstants.POSTGRESQL_EXECUTE_QUERY, url, normalizedUrl, hostList, databaseId);
